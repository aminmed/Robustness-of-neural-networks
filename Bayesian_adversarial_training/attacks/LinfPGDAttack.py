...
class LinfPGDAttack(object):
    def __init__(self, model,epsilon = 8/255.,alpha=2/255,iteration=40):
        self.model = model
    ...
    def perturb(self, x_natural, y):
        ...
        for i in range(self.iteration):
            ...
            adv_images = x.detach() + self.alpha * torch.sign(grad.detach())
...
def Linf_PGD(x_in, y_true, net, steps, eps):
    attack = LinfPGDAttack(net, eps, 2/255, steps, True)
    x_adv = attack.perturb(x_in, y_true)
    return x_adv 
...
def main() : 
    ...
    parser.add_argument("experiment_dir", metavar="experiment-dir", nargs="?", default=os.path.join(os.getcwd(), "experiments/LinfPGD"),
                        help="Path to save experiment.")
    
    parser.add_argument("-b", "--batch-size", type=int, default=256,
                        help="Set batch size.")
    parser.add_argument("-e", "--epsilon", type=float, default=0.0314,
                        help="Set epsilon (noise parameter).")
    parser.add_argument("-a", "--alpha", type=float, default=0.01,
                        help="Set alpha (noise parameters).")
    args = parser.parse_args()
    batch_size = args.batch_size
    experiment_directory = os.path.join(args.experiment_dir,\
                                        'LinfPGD_eps_' + str(args.epsilon * 255.)[:4]+\
    ...
    print("Start LinfPGD attack ...")
    LinfPGD_model = LinfPGDAttack(model = net, epsilon = args.epsilon, alpha=args.alpha,iteration=40)
    adverserial_images = []
    y_preds = []
    y_preds_adv = []
    adv_confidences = []
    ...
    for i, (data, targets) in tqdm(enumerate(te_loader)) : 
         ...
         adv_data , adv_outputs = LinfPGD_model.perturb(data, targets)
