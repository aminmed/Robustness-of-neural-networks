...
### This Function has been changed to add the adversial attack during the test
def test_natural(net, test_loader, num_samples,adversary):
    correct = 0
    total = 0
    ...
    ## We return the acc before and after the attack.
    ###This is before doing adversial training
    return  100*correct / total,  100*adv_correct / total
...
def main():
    ...
    acc_nat = test_natural(net, valid_loader, num_samples = args.num_samples,adversary=adversary)
    print("Model nat accuracy (test): {}".format(acc_nat[0]),"Model nat accuracy with attack(test): {}".format(acc_nat[1]))
